...
import graphqlRequest, { GraphQLError } from "../../lib/gqlRequest"
...
export default class UserCommand extends SubCommand {
    async run(ctx: CommandContext<typeof denyOptions>) {
        if (ctx.options.reason.startsWith("preset:")) {
            const preset = denialReasonsPresets.find(p => p.id === Number(ctx.options.reason.split(":")[1]))
        ...
        const req = await graphqlRequest<MutationResponseData | GraphQLError>(query, { input: { id: ctx.options.id, reason: ctx.options.reason } }, "Deny")
